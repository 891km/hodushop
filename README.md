...
- **상품/구매 플로우**
  ...
  - 메인 목록(`src/pages/index/index.js`): 상품 리스트 렌더링, 이미지 에러 대비, 커스텀 배너 스와이퍼/페이지네이션/반응형 리사이즈 처리
...
- **공용 UI/UX**
  - 헤더/GNB(`src/components/header.js`): 로그인 상태/역할에 따라 액션 버튼 동적 구성(장바구니/마이페이지/판매자 센터), 비로그인 장바구니 접근 시 모달 안내, 드롭다운 위치 보정, 상품 검색 기능, 검색어 클리어 기능
...
- **해결:**
  ...
  - 조건별로 각 기능(장바구니, 마이페이지 드롭다운, 로그아웃, 검색어 초기화)을 나누어 호출
